...
export async function POST(request: NextRequest) {
    try {
        ...
        console.log("Test Generate Meet - Session debug:", {
            hasSession: !!session,
        ...
        if (!session) {
            return NextResponse.json(
                {
                    success: false,
                    message: "Please sign in with Google first to generate meeting links.",
                    error: "NO_SESSION"
                },
                { status: 401 }
            );
        ...
        if (!bookingId || !mentorEmail || !studentEmail || !date || !time) {
            return NextResponse.json(
                {
                    success: false,
                    message: "Missing required fields: bookingId, mentorEmail, studentEmail, date, time"
                },
                { status: 400 }
            );
        }
        ...
        if (error instanceof Error) {
            if (error.message.includes("нэвтрэх эрх")) {
                ...
                errorMessage = "Google authentication failed. Please sign in with Google again.";
            } else {
