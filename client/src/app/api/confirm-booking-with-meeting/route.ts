...
export async function POST(request: NextRequest) {
  try {
    ...
    const meetingData = {
      ...
      description:
        description ||
        `${mentorEmail} болон ${studentEmail} хоорондын хичээл. Захиалгын ID: ${bookingId}`,
    };
    ...
    try {
      ...
      if (!backendUpdateSuccess) {
        console.warn(
          "Failed to update booking in backend, but meeting was created:",
    ...
    if (error instanceof Error) {
      if (error.message.includes("нэвтрэх эрх")) {
        ...
        errorMessage =
          "Түр хугацаанд хэт олон хүсэлт илгээгдсэн. Дахин оролдоно уу.";
      } else {
