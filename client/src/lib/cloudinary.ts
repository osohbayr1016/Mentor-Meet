...
// Fallback function to create local blob URL (for testing when Cloudinary is not available)
const createLocalImageUrl = (file: File): string => {
  const blobUrl = URL.createObjectURL(file);
  console.warn(
    "Using local blob URL as fallback. This is for testing only and will not persist."
  );
  return blobUrl;
};
